...
public class HttpProxyConfigFactory {
	...
	public static HttpProxyConfig getHttpProxyConfig(ProxyInfo info) {
		...
		if (provider instanceof HttpProxy) {
			...
			config.setHost(proxyProvider.getHost());
			config.setPort(proxyProvider.getPort());
			if (StringUtils.isNotEmpty(proxyProvider.getLogin())) {
				config.setLogin(proxyProvider.getLogin());
				config.setPass(proxyProvider.getPass());
			}
		}
	...
	public static ProxyInfo toProxyInfo(HttpProxyConfig config) throws ConfigurationLoadingException {
		...
		case "HttpProxy":
			provider = new HttpProxy(config.getHost(), config.getPort(), config.getLogin(), config.getPass());
			break;
		case "NoProxy":
